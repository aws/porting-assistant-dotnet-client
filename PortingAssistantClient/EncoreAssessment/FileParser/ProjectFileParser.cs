...
using Buildalyzer;
using EncoreAssessment.ErrorHandle;
using EncoreCommon.Model;
using EncoreCommon.Utils;
using NuGet.Packaging;
using NuGet.Versioning;

namespace EncoreAssessment.FileParser
{
    public class ProjectFileParser
    {
        ...
        public List<ProjectReference> GetProjectReferences()
        {
            return _projectElement.GetDescendants(ProjectReference).Select(s =>
                EncoreCommon.Model.ProjectReference.Get(s, _path)).ToList();
        }
        ...
        private List<PackageReference> ProcessPackagesConfigFile()
        {
            ...
            {
                throw new EncoreAssessmentException("Unable to parse packages.config file", ex); 
            }
        }
    }
}
